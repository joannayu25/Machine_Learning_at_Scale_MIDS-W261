...
ignore = ['I-12-Scaled', 'C-22-Index', 'Label']
...
temp = ['Label']
for i in range(1,14): temp.append('I-'+str(i)+'-Scaled')
for i in range(1,27): temp.append('C-'+str(i)+'-Index')
print(temp)
toyDF = scaler_indexers(assembler_indexers(string_indexers(toyDF)))
toyDF_trans = vector_transform(toyDF.select(temp), ignore)
toyDF_trans.show(5)

miniValDF = scaler_indexers(assembler_indexers(string_indexers(miniValDF)))
...
RF_SSI_PD = pd.DataFrame(RF_results, columns=['# of Trees', 'Log Loss', 'Area Under ROC', 'Area Under PR', 'Wall Time'])
print(RF_SSI_PD)
print(RF_results)
